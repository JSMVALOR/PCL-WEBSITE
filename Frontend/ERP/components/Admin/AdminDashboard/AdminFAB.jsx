...
export default function AdminFAB({}) {
 ...
 key={i} 
 className="flex items-center gap-3 bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 p-2 pr-4 rounded-lg hover:border-themeAccent hover:text-themeAccent transition group"
 style={{ transitionDelay: `${(actions.length - i) * 30}ms` }}
 >
 <div className="w-8 h-8 rounded-lg bg-themeElevated/90 backdrop-blur-2xl flex items-center justify-center text-themeTextSec group-hover:bg-themeAccent group-hover:text-themeText dark:text-white transition-colors">
 ...
 className={`w-14 h-14 rounded-lg bg-themeAccent text-themeText dark:text-white flex items-center justify-center transition-transform hover:scale-110 active:scale-95 ${isOpen ? 'rotate-45' : 'rotate-0'}`}
 >
