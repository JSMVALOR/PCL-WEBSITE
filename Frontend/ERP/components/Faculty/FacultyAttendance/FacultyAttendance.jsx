    ...
    try {
        ...
        if (cohortSubs && cohortSubs.length > 0) {
            const { data: rawSessions, error: sesError } = await supabase
                .from('class_sessions')
                .select('id, subject_id, present_count, total_students, schedule_id, date, status')
                .eq('faculty_id', userSession.db_id)
...
{filteredStudents.map((student, index) => {
 ...
 const currentStatus = (editMode && stagedChanges[student.id]) ? stagedChanges[student.id] : (attendancePhase === 'entry' ? record.entry_status : record.exit_status);
        ...
        <div className="hidden lg:flex items-center justify-between p-4 hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl transition-colors border-b border-black/5 dark:border-white/5 last:border-0 group">
            ...
            <div className="flex bg-black/5 dark:bg-white/10 p-1 rounded-xl border border-black/5 dark:border-white/5 shrink-0">
                ...
                if (isPhaseOneSkipped) {
                    return (
                        <>
                            <button type="button" onClick={() => handleAction('present')} disabled={activeSession.status === 'completed' && !editMode} className={`w-20 h-9 rounded-lg text-[12px] font-bold tracking-tight transition-all ${record.exit_status === 'present' ? 'bg-white dark:bg-themeElevated text-emerald-500 shadow-sm' : 'text-themeTextSec hover:text-themeText dark:hover:text-themeText'}`}>Stayed</button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050505]/95 backdrop-blur-md p-4 animate-fade-in no-print">
        <div className="bg-gradient-to-b from-[#1a1a1a] to-[#121212] border border-[#2a2a2a] p-10 rounded-2xl flex flex-col items-center max-w-lg w-full relative">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-[0_0_60px_rgba(255,255,255,0.1)] relative">
                <QRCode value={JSON.stringify(qrCodeData)} size={280} level="H" includeMargin={false} />
            </div>
...
{activeTab === 'analytics' && (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8 bg-white/40 dark:bg-themePanel/20 backdrop-blur-xl border border-black/5 dark:border-white/5 rounded-[2rem] shadow-sm animate-fade-in">
        <div className="w-24 h-24 rounded-full bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-5xl mb-6 border border-emerald-500/20 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
            <i className="fa-solid fa-shield-check"></i>
        </div>
        <h3 className="text-2xl text-themeText dark:text-white font-black tracking-tight">All Clear!</h3>
        <p className="text-base font-medium text-themeTextSec mt-2 max-w-md">None of your students are currently falling below the critical 75% attendance threshold across your active courses.</p>
        
        <div className="flex flex-col sm:flex-row items-center gap-6 mt-10 p-6 bg-black/[0.02] dark:bg-white/[0.02] rounded-2xl border border-black/5 dark:border-white/5 w-full max-w-lg">
            <div className="flex-1 border-b sm:border-b-0 sm:border-r border-black/10 dark:border-white/10 pb-4 sm:pb-0 sm:pr-6 text-center w-full sm:w-auto">
                <p className="text-[10px] font-black uppercase tracking-widest text-themeTextSec">At Risk Students</p>
                <p className="text-3xl font-black text-themeText dark:text-white mt-1">0</p>
            </div>
            <div className="flex-1 border-b sm:border-b-0 sm:border-r border-black/10 dark:border-white/10 pb-4 sm:pb-0 sm:pr-6 text-center w-full sm:w-auto">
                <p className="text-[10px] font-black uppercase tracking-widest text-themeTextSec">Avg Engagement</p>
                <p className="text-3xl font-black text-emerald-500 mt-1">94%</p>
            </div>
            <div className="flex-1 text-center w-full sm:w-auto pt-2 sm:pt-0">
                <p className="text-[10px] font-black uppercase tracking-widest text-themeTextSec">Classes Monitored</p>
                <p className="text-3xl font-black text-themeText dark:text-white mt-1">{allSubjects.reduce((acc, sub) => acc + (sub.classesDone || 0), 0)}</p>
            </div>
        </div>
    </div>
)}
