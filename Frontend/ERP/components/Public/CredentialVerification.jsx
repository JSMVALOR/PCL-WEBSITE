                ...
                <div className="flex flex-col items-center justify-center mb-12 text-center">
                    <div className="w-20 h-20 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(16,185,129,0.15)] relative">
                        ...
                        <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-blue-500 rounded-full border-[3px] border-[#0a0a0a] flex items-center justify-center">
                            <i className="fa-solid fa-badge-check text-themeText dark:text-white text-[10px]"></i>
                        </div>
                    </div>
                    <h1 className="text-3xl sm:text-4xl font-black text-themeText dark:text-white tracking-tight mb-2">Official University Record</h1>
                ...
                <div className="bg-[#111] border border-neutral-800 rounded-2xl p-6 sm:p-8 mb-8 shadow-2xl">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        <div>
                            ...
                            <p className="text-themeText dark:text-white font-medium text-lg">{profile.full_name}</p>
                        </div>
                        <div>
                            <p className="text-neutral-500 text-xs font-bold tracking-normal mb-1">ERP Registration ID</p>
                            <p className="text-themeText dark:text-white font-medium text-lg font-mono">{profile.erp_id || 'Pending'}</p>
                        </div>
                        <div>
                            <p className="text-neutral-500 text-xs font-bold tracking-normal mb-1">Program of Study</p>
                            <p className="text-themeText dark:text-white font-medium text-lg">{profile.department || 'B.B.A. LL.B. (Hons.)'}</p>
                        </div>
                        <div>
                            <p className="text-neutral-500 text-xs font-bold tracking-normal mb-1">Academic Batch</p>
                            <p className="text-themeText dark:text-white font-medium text-lg">{profile.academic_batch || 'Unassigned'}</p>
                        </div>
                    </div>
                </div>

                {/* Verified Documents Vault */}
                <h3 className="text-xl font-bold text-themeText dark:text-white mb-6 flex items-center gap-3">
                        ...
                        documents.map(doc => (
                            <div key={doc.id} className="bg-[#111] border border-neutral-800 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-emerald-500/50 transition-colors group">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-lg bg-themeApp border border-neutral-800 flex items-center justify-center shrink-0 group-hover:bg-emerald-500/10 group-hover:border-emerald-500/30 transition-colors">
                                    ...
                                    <div>
                                        <h4 className="text-themeText dark:text-white font-medium mb-1 line-clamp-1">{doc.document_name}</h4>
                                ...
                                <button type="button" 
                                    ...
                                    className="w-full sm:w-auto px-5 py-2.5 bg-themeApp hover:bg-emerald-500 hover:text-themeApp text-neutral-300 rounded-lg text-xs font-bold tracking-normal transition border border-neutral-800 hover:border-emerald-500 shrink-0"
                                >
