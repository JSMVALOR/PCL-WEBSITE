...
export default function SlideCommit({
  ...
  const travel = width - handleSize - 12;
  ...
  const handleDragEnd = (e, info) => {
    ...
    // If pulled more than 75% or flicked hard right
    if (x.get() >= travel * 0.75 || info.velocity.x > 300) {
      animate(x, travel, {
        type: 'spring',
        stiffness: 400,
        damping: 30,
  ...
  return (
    ...
    >
      {/* Dark mode track via pseudo-element if needed, or just rely on CSS */}
...
`;
fs.writeFileSync('src/Shared/components/ReactBits/SlideCommit/SlideCommit.jsx', content);
console.log("SlideCommit patched.");
