...
async function run() {
    let res = await supabase.from('mentorship_groups').select('*').limit(1);
    console.log("mentorship_groups:", res.error ? res.error.message : Object.keys(res.data[0] || {}));
    
    let res2 = await supabase.from('mentees').select('*').limit(1);
    console.log("mentees:", res2.error ? res2.error.message : Object.keys(res2.data[0] || {}));
}
run();
