...
import legalAidImg from '../../../../../Shared/Assets/CAMPUS/pcl_legal_clinic.webp';
import styles from '../../PROGRAMS/Programs.module.css';

gsap.registerPlugin(ScrollTrigger);

const LEGAL_AID_CONTENT = {
  heroSubtitle: "The Legal Aid Clinic is the social heart of Prudentia College of Law. It empowers students to apply their legal knowledge in real-world scenarios while providing crucial pro-bono assistance to marginalized communities.",
  cards: [
    {
      title: "Pro-Bono Counseling",
      text: "Students work alongside empanelled advocates to provide free legal counseling to socio-economically weaker sections of society.",
      icon: "Handshake"
    },
    {
      title: "Awareness Camps",
      text: "Organizing legal literacy camps in rural and underserved areas to educate citizens about their fundamental rights and legal remedies.",
      icon: "Megaphone"
    },
    {
      title: "Lok Adalat Coordination",
      text: "Active participation in alternative dispute resolution mechanisms and Lok Adalats, fostering a culture of amicable settlement.",
      icon: "Scale"
    }
  ]
};
      ...
      <div className={styles.contentContainer} ref={contentRef}>
        ...
        <div className="mb-16 relative z-10 gsap-fade-up">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="px-4 py-1 border border-[var(--accent)]/30 rounded-full text-[var(--accent)] text-xs md:text-sm tracking-widest uppercase bg-[var(--accent)]/10 backdrop-blur-sm font-bold whitespace-nowrap">Community Outreach</span>
        ...
        <div className="w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden mb-20 relative flex items-center justify-center group shadow-[0_30px_60px_rgba(0,0,0,0.4)] border border-[var(--card-border)] gsap-fade-up z-10">
          <img decoding="async" loading="lazy" src={legalAidImg} alt="Legal Aid Clinic" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700" />
        ...
        <div className="grid md:grid-cols-3 gap-8 relative z-10">
          {LEGAL_AID_CONTENT.cards.map((card, index) => {
            const IconComponent = Icons[card.icon] || Icons.Scale;
        ...
        <div className="mt-24 text-center gsap-fade-up relative z-10 flex flex-col items-center">
          <Link to="/contact" className="tlh-btn justify-center" style={{ maxWidth: '300px' }}>
            <span className="text-xs font-bold uppercase tracking-widest">Seek Legal Assistance</span>
