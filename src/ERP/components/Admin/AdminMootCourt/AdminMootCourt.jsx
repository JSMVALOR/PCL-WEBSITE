...
export default function AdminMootCourt({ isHubView = false , isEmbedded = false}) {
...
<button type="button"
 ...
 moots.map(moot => (
 <div key={moot.id} className="bg-themePanel/85 backdrop-blur-2xl p-5 rounded-themePanel border border-white/5 flex justify-between items-center">
 <div>
 ...
 return (
 <div key={bid.id} className={`bg-themePanel/85 backdrop-blur-2xl p-6 rounded-themePanel border-theme ${isSelected ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-white/5'}`}>
 <div className="flex justify-between items-start mb-4">
 <div>
 <span className="text-[10px] font-black uppercase tracking-widest text-themeAccent bg-themeElevated/90 backdrop-blur-2xl px-2 py-1 rounded-md border border-black/5 dark:border-white/10">
 {mootName}
 </span>
 ...
 <input type="text" value={mootForm.moot_name} onChange={e => setMootForm({...mootForm, moot_name: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none focus:border-themeAccent" required />
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-[10px] font-black uppercase tracking-widest text-themeTextSec mb-2 ml-1">Level</label>
 <select value={mootForm.level} onChange={e => setMootForm({...mootForm, level: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none">
 ...
 <input type="text" value={mootForm.venue} onChange={e => setMootForm({...mootForm, venue: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none" required />
 </div>
 <div>
 <label className="block text-[10px] font-black uppercase tracking-widest text-themeTextSec mb-2 ml-1">Description</label>
 <textarea rows="3" value={mootForm.description} onChange={e => setMootForm({...mootForm, description: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none resize-none" required></textarea>
 </div>
 <button type="submit" disabled={isSubmitting} className="w-full py-4 mt-2 bg-amber-500 hover:bg-amber-400 text-[#050505] rounded-lg text-[10px] font-black uppercase tracking-widest transition active:scale-[0.98]">
