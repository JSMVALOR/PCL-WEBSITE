...
export default function AdminMootCourt({ isEmbedded = false,  isHubView = false }) {
 ...
 try {
 const { data, error } = await supabase.from('moot_competitions').select('*').order('created_at', { ascending: false });
 if (!error && data) {
 setMoots(data);
 }
 } catch (error) {
...
<button type="button"
 ...
 {moots.length === 0 ? (
 <div className="p-12 text-center text-themeTextSec bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-themePanel">
 <i className="fa-solid fa-gavel text-4xl mb-4 opacity-50"></i>
 <p>No active moot competitions.</p>
 </div>
 ) : (
 moots.map(moot => (
 <div key={moot.id} className="bg-themePanel/85 backdrop-blur-2xl p-5 rounded-themePanel border border-themeBorder dark:border-white/5 flex justify-between items-center">
 <div>
 ...
 return (
 <div key={bid.id} className={`bg-themePanel/85 backdrop-blur-2xl p-6 rounded-themePanel border-theme ${isSelected ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-themeBorder dark:border-white/5'}`}>
 ...
 <div className="bg-themeApp p-4 rounded-lg border border-themeBorder dark:border-white/5 mb-4">
 ...
 <div className="bg-themeApp w-full max-w-lg rounded-themePanel overflow-hidden border border-themeBorder dark:border-white/5 flex flex-col max-h-[90vh]">
 <div className="bg-themePanel/85 backdrop-blur-2xl p-6 border-b-theme border-themeBorder dark:border-white/5 flex justify-between items-center">
 ...
 <input type="text" value={mootForm.moot_name} onChange={e => setMootForm({ ...mootForm, moot_name: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none focus:border-themeAccent" required />
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-[13px] font-medium text-themeTextSec mb-2 ml-1">Level</label>
 <select value={mootForm.level} onChange={e => setMootForm({ ...mootForm, level: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none">
 ...
 <input min="2026-09-14" type="date" value={mootForm.event_date} onChange={e => setMootForm({ ...mootForm, event_date: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none" required />
 </div>
 </div>
 <div>
 <label className="block text-[13px] font-medium text-themeTextSec mb-2 ml-1">Venue</label>
 <input type="text" value={mootForm.venue} onChange={e => setMootForm({ ...mootForm, venue: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none" required />
 </div>
 <div>
 <label className="block text-[13px] font-medium text-themeTextSec mb-2 ml-1">Description</label>
 <textarea rows="3" value={mootForm.description} onChange={e => setMootForm({ ...mootForm, description: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-themeBorder dark:border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none resize-none" required></textarea>
 </div>
