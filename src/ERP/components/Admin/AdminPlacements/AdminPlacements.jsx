...
export default function AdminPlacements({ isHubView = false, isEmbedded = false }) {
...
<button
 ...
 {drives.length === 0 ? (
 <div className="p-12 text-center text-themeTextSec bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-themePanel">
 <i className="fa-solid fa-building text-4xl mb-4 opacity-50"></i>
 <p>No active placement drives.</p>
 </div>
 ) : (
 drives.map(drive => (
 <div key={drive.id} className="bg-themePanel/85 backdrop-blur-2xl p-6 rounded-themePanel border border-white/5 flex flex-col gap-4 relative overflow-hidden group">
 ...
 className="px-4 py-2 bg-themeElevated/90 backdrop-blur-2xl hover:bg-neutral-800 border border-black/5 dark:border-white/10 rounded-lg text-[10px] font-black text-themeTextSec uppercase tracking-widest transition-colors"
 >
 Toggle Status
 </button>
 </div>
 ...
 return (
 <div key={app.id} className="bg-themePanel/85 backdrop-blur-2xl p-5 rounded-themePanel border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6">
 ...
 <span className="text-[10px] font-bold text-themeTextSec uppercase tracking-widest px-2 py-1 bg-themeElevated/90 backdrop-blur-2xl rounded border border-black/5 dark:border-white/10">
 {role}
 </span>
 </div>
 ...
 app.status === 'Applied' ? 'bg-themeElevated/90 backdrop-blur-2xl text-themeText border-black/5 dark:border-white/10' :
 ...
 inquiries.map(inq => (
 <div key={inq.id} className="bg-themePanel/85 backdrop-blur-2xl p-5 rounded-themePanel border border-white/5 flex flex-col md:flex-row md:items-start justify-between gap-6">
 <div className="flex-1">
 <div className="flex items-center gap-2 mb-2">
 ...
 <input type="text" value={driveForm.company_name} onChange={e => setDriveForm({...driveForm, company_name: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none focus:border-themeAccent transition-colors" required />
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-[10px] font-black uppercase tracking-widest text-themeTextSec mb-2 ml-1">Role Title</label>
 <input type="text" value={driveForm.role_title} onChange={e => setDriveForm({...driveForm, role_title: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none focus:border-themeAccent transition-colors" required />
 </div>
 <div>
 ...
 <textarea rows="2" value={driveForm.eligibility_criteria} onChange={e => setDriveForm({...driveForm, eligibility_criteria: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none resize-none focus:border-themeAccent transition-colors" placeholder="e.g. Min 7.0 CGPA, No active backlogs" required></textarea>
 </div>
 <div>
 <label className="block text-[10px] font-black uppercase tracking-widest text-themeTextSec mb-2 ml-1">Package Details</label>
 <textarea rows="2" value={driveForm.package_details} onChange={e => setDriveForm({...driveForm, package_details: e.target.value})} className="w-full bg-themePanel/85 backdrop-blur-2xl border border-white/5 rounded-lg px-4 py-3 text-xs font-bold text-themeText outline-none resize-none focus:border-themeAccent transition-colors" placeholder="e.g. 12 LPA CTC, 6-month internship + PPO" required></textarea>
 </div>
