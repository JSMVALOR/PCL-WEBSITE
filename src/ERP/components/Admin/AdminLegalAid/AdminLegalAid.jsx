...
export default function AdminLegalAid({ isEmbedded = false,  isHubView = false }) {
 ...
 <div className="bg-themePanel/85 backdrop-blur-2xl rounded-themePanel border border-gray-200 dark:border-white/5 p-6">
 ...
 <div className="text-center py-12 border-2 border-dashed border-gray-200 dark:border-white/5 rounded-themePanel">
 ...
 {diaries.map(d => (
 <div key={d.id} className="bg-themeElevated/90 backdrop-blur-2xl p-5 rounded-themePanel border border-gray-200 dark:border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
 ...
 {d.status === 'pending' && (
 <>
 <button type="button" onClick={() => handleStatusUpdate(d.id, 'approved')} className="px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-gray-900 dark:text-white font-bold text-xs rounded transition-colors flex-1 text-center">
 <i className="fa-solid fa-check mr-1"></i> Approve
 </button>
 <button type="button" onClick={() => handleStatusUpdate(d.id, 'rejected')} className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-gray-900 dark:text-white font-bold text-xs rounded transition-colors flex-1 text-center">
 <i className="fa-solid fa-xmark mr-1"></i> Reject
 </button>
 </>
 )}
 {d.status !== 'pending' && (
 <button type="button" onClick={() => handleStatusUpdate(d.id, 'pending')} className="px-4 py-2 bg-themePanel/85 backdrop-blur-2xl border border-gray-200 dark:border-white/5 hover:bg-themeElevated/90 backdrop-blur-2xl text-themeTextSec font-bold text-xs rounded transition-colors w-full text-center">
 Reset Status
 </button>
 )}
 </div>
