...
export default function AdminUserEditorModal({ user, isOpen, onClose, onUpdate }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className={`${theme.layout.panel} rounded-2xl w-full max-w-lg p-6 lg:p-8 flex flex-col gap-6 shadow-2xl relative border border-gray-300 dark:border-white/10`}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 overflow-y-auto max-h-[70vh] pr-2">
          ...
          <div className="flex justify-end gap-3 mt-4">
            <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl text-xs font-bold text-themeTextSec hover:bg-themeElevated/90 transition-colors border border-transparent hover:border-black/5 dark:hover:border-gray-300 dark:border-white/10">
              Cancel
            </button>
            <button type="submit" disabled={isSaving} className="px-5 py-2.5 rounded-xl text-xs font-bold bg-themeAccent text-gray-900 dark:text-white hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center gap-2">
