...
export default function LeaveCalendar({}) {
 ...
 days.push(<div key={`blank-${i}`} className="min-h-[100px] border border-gray-200 dark:border-white/5/50 bg-black/5 dark:bg-white/5/30 rounded-xl"></div>);
 }
 ...
 ${isToday ? 'border-indigo-500 bg-indigo-500/5' : 'border-gray-200 dark:border-white/5 bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-3xl saturate-[1.8]/85 backdrop-blur-2xl hover:bg-black/5 dark:bg-white/5/90 backdrop-blur-2xl hover:border-black/5 dark:border-white/10'}
 `}>
 ...
 <div key={leave.id} className={`text-[8px] lg:text-[9px] font-bold px-1.5 py-0.5 lg:px-2 lg:py-1 rounded truncate bg-${leave.leave_type || 'blue'}-500/10 text-${leave.leave_type || 'blue'}-500 hover:bg-${leave.leave_type || 'blue'}-500 hover:text-gray-900 dark:text-white transition-colors cursor-default`}>
 {'Faculty' || 'Faculty'}
 </div>
 ))}
 </div>
 ...
 <div className="flex flex-col md:flex-row items-center justify-between bg-white/60 dark:bg-[#1C1C1E]/60 backdrop-blur-3xl saturate-[1.8] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-black/[0.04] dark:border-white/[0.08] rounded-[2rem] p-4 lg:p-5 gap-4">
 ...
 {isLoading && (
 <div className="absolute inset-0 z-10 bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-3xl saturate-[1.8]/50 backdrop-blur-sm flex flex-col items-center justify-center rounded-xl">
