...
export default function LeavePolicies({ isEmbedded = false }) {
 ...
 <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.erpDialog?.alert("Development in Progress: This module is scheduled for Phase 2 deployment."); }} className="w-full md:w-auto px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition flex items-center justify-center gap-2">
 ...
 <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.erpDialog?.alert("Development in Progress: This module is scheduled for Phase 2 deployment."); }} className="text-themeTextSec hover:text-indigo-500 transition-colors w-8 h-8 flex items-center justify-center rounded-lg hover:bg-indigo-500/10">
