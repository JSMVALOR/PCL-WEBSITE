...
export default function SyllabusEditorModal({ subject, onClose, onRefresh, isReadOnly = false }) {
    ...
    const modalContent = (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/10 shrink-0">
                    <div>
                        ...
                        <p className="text-xs font-bold text-gray-500 dark:text-white/50 mt-1">{subject.name}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-black text-gray-900 dark:text-white border-b border-gray-100 dark:border-white/10 pb-2">Overview</h3>
                        <div>
                            <label className="text-[10px] font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest block mb-1">Course Objectives</label>
                            <textarea 
                                readOnly={isReadOnly}
                                rows={6}
                                value={objectives} 
                                onChange={e => setObjectives(e.target.value)} 
                                className="w-full bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-gray-900 dark:text-white/90 outline-none focus:border-amber-500" 
                                placeholder="E.g., CO1: To introduce..."
                            />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest block mb-1">Learning Outcomes</label>
                            <textarea 
                                readOnly={isReadOnly}
                                rows={6}
                                value={outcomes} 
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/10 pb-2">
                            <h3 className="text-sm font-black text-gray-900 dark:text-white">Course Content (Units)</h3>
                            {!isReadOnly && (
                        ...
                        {Object.keys(content).sort((a, b) => {
                            ...
                            <div key={key} className="bg-gray-50 dark:bg-black border border-gray-200 dark:border-white/10 rounded-xl p-4 relative group flex flex-col gap-2">
                                <div className="flex justify-between items-center mb-1">
                                    <label className="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{key}</label>
                                    {!isReadOnly && (
                                        <button onClick={() => removeUnit(key)} className="text-rose-500 hover:bg-rose-500 hover:text-white w-6 h-6 rounded flex items-center justify-center transition">
                                ...
                                <textarea 
                                    ...
                                    className="w-full bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm font-medium text-gray-900 dark:text-white/90 outline-none focus:border-amber-500" 
                                />
                            </div>
                        ))}
                        {Object.keys(content).length === 0 && (
                            <p className="text-xs text-gray-500 dark:text-white/50 italic text-center py-4">No units defined.</p>
                        )}
                    </div>

                    {/* Readings and Cases */}
                    <div className="space-y-4">
                        <h3 className="text-sm font-black text-gray-900 dark:text-white border-b border-gray-100 dark:border-white/10 pb-2">References</h3>
                        <div>
                            <label className="text-[10px] font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest block mb-1">Suggested Case Laws</label>
                            <textarea 
                                readOnly={isReadOnly}
                                rows={6}
                                value={caseLaws} 
                        ...
                        <div>
                            <label className="text-[10px] font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest block mb-1">Suggested Readings</label>
                            <textarea 
                                readOnly={isReadOnly}
                                rows={6}
                                value={readings} 
                ...
                <div className="p-6 border-t border-gray-100 dark:border-white/10 shrink-0 flex gap-3">
                    <button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold text-sm bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-white/70 hover:bg-gray-200 dark:hover:bg-white/20 transition">
