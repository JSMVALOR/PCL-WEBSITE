...
export default function Fees({ isEmbedded = false }) {
 ...
 <div className="bg-themePanel border border-black/5 dark:border-white/10 p-8 rounded-2xl border border-black/5 dark:border-white/10 flex flex-col items-center max-w-sm w-full mx-4">
 <i className="fa-solid fa-circle-notch fa-spin text-4xl text-themeAccent mb-6"></i>
 <h3 className={`${theme.text.heading} text-xl mb-2 text-center text-themeText dark:text-white`}>Processing Payment</h3>
 ...
 <div className="bg-themePanel border border-black/5 dark:border-white/10 shadow-none rounded-[2rem] p-6 lg:p-8 max-w-md w-full relative">
 <button onClick={() => setIsVerificationModalOpen(false)} className="absolute top-6 right-6 text-themeTextSec dark:text-white/50 hover:text-themeText dark:text-white transition-colors">
 <i className="fa-solid fa-xmark text-xl"></i>
 </button>
 <h2 className="text-xl font-black text-themeText dark:text-white mb-2">Verify Payment</h2>
 <p className="text-xs font-bold text-themeTextSec dark:text-white/50 uppercase tracking-widest mb-6">Submit your transaction details for admin verification.</p>
     ...
     <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 mb-2 animate-fade-in flex items-center justify-between">
         <div>
             ...
             <div className="flex items-center gap-2 mt-2">
                <p className="text-sm font-black text-themeText dark:text-white font-mono tracking-tight">prudentia@icici</p>
 ...
 {verificationData.mode === 'Demand Draft' && (
     <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-2 animate-fade-in">
         ...
         <p className="text-xs text-themeTextSec dark:text-white/70 leading-relaxed font-medium">Draw the Demand Draft in favor of <strong className="text-themeText dark:text-white font-black">"PRUDENTIA COLLEGE OF LAW"</strong>, payable at <strong className="text-themeText dark:text-white font-black">Hyderabad</strong>. Submit the physical DD to the accounts office.</p>
     </div>
 )}
 
 <form onSubmit={handleVerificationSubmit} className="flex flex-col gap-4">
 <div>
 <label className="text-[10px] font-bold text-themeTextSec dark:text-white/50 uppercase tracking-widest block mb-1">Mode of Payment</label>
 <select value={verificationData.mode} onChange={e => setVerificationData({...verificationData, mode: e.target.value})} className="w-full bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-themeText dark:text-white outline-none focus:border-themeBorder dark:border-white/5Accent appearance-none">
 ...
 <div>
 <label className="text-[10px] font-bold text-themeTextSec dark:text-white/50 uppercase tracking-widest block mb-1">Reference No. / UTR</label>
 <input type="text" required value={verificationData.referenceNumber} onChange={e => setVerificationData({...verificationData, referenceNumber: e.target.value})} className="w-full bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-themeText dark:text-white outline-none focus:border-themeBorder dark:border-white/5Accent" placeholder="Enter Transaction ID" />
 </div>
 <div>
 ...
 <div className="bg-themePanel border border-black/5 dark:border-white/10 p-8 rounded-2xl border-themeBorder dark:border-white/5 border-emerald-500/30 flex flex-col items-center max-w-sm w-full mx-4">
 ...
 <h3 className={`${theme.text.heading} text-xl mb-2 text-center text-themeText dark:text-white`}>{successModal?.isPending ? 'Verification Pending' : 'Payment Successful'}!</h3>
 <p className={`${theme.text.secondary} text-sm text-center mb-6`}>
 {successModal?.isPending ? 'Your payment details of' : 'Your payment of'} <span className="font-bold text-themeText dark:text-white">{formatCurrency(successModal.amount)}</span> has been securely processed.
 </p>
 ...
 <span className="text-themeTextSec dark:text-white/50">Transaction ID</span>
 <span className="font-mono text-themeText dark:text-white">{successModal.transactionId}</span>
 </div>
 <div className="flex justify-between text-xs">
 <span className="text-themeTextSec dark:text-white/50">Date</span>
 <span className="font-mono text-themeText dark:text-white">{new Date().toLocaleDateString('en-GB')}</span>
 </div>
 </div>
 <button type="button" 
 onClick={() => setSuccessModal(null)}
 className="w-full py-3 bg-black/5 dark:bg-white/5 backdrop-blur-xl border border-black/10 dark:border-white/5 hover:bg-black/5 dark:bg-white/5 text-themeText dark:text-white text-sm font-bold rounded-lg hover:border-amber-500/50 transition duration-300"
 >
 Back to Ledger
 </button>
 </div>
 ...
 <div className={`flex p-1.5 bg-themePanel border border-black/5 dark:border-white/10 rounded-2xl lg:rounded-2xl w-full lg:w-fit overflow-x-auto no-scrollbar`}>
 <button type="button"
 ...
 : "text-themeTextSec dark:text-white/50 hover:text-themeText dark:text-white border border-transparent"
 }`}
 >
 Outstanding Dues
 </button>
 ...
 <div className={`bg-themePanel border border-black/5 dark:border-white/10 rounded-[2rem] p-6 lg:p-8 flex flex-col justify-between relative overflow-hidden`}>
 <div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div>
 <div>
 <p className="text-[13px] font-medium text-themeTextSec dark:text-white/50 mb-2">Total Expected Fee</p>
 <h2 className="text-3xl font-semibold tracking-tight text-themeText dark:text-white font-mono">{formatCurrency(totalExpected)}</h2>
 </div>
 </div>
 ...
 <div>
 <p className="text-[13px] font-medium text-themeTextSec dark:text-white/50 mb-2">Amount Paid</p>
 <h2 className="text-3xl font-semibold tracking-tight text-emerald-400 font-mono">{formatCurrency(totalPaid)}</h2>
 </div>
 <div className="w-12 h-12 rounded-full border-[4px] border-themeBorder dark:border-white/5 flex items-center justify-center relative">
 ...
 <span className="text-[9px] font-black text-themeText dark:text-white">{progressPercent}%</span>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-[13px] font-medium text-themeTextSec dark:text-white/50 mb-2">Outstanding Dues</p>
 ...
 {/* The Master Card */}
 <div className="bg-themePanel border border-black/5 dark:border-white/10 rounded-2xl p-6 lg:p-8 relative overflow-hidden text-themeText dark:text-white flex flex-col justify-between min-h-[300px] lg:min-h-[350px] group hover:border-amber-500/30 transition duration-500">
 {/* Background Glows */}

 <div className="relative z-10 flex justify-between items-start">
 <div className={`w-12 h-12 lg:w-14 lg:h-14 rounded-2xl flex items-center justify-center border border-black/5 dark:border-white/10 bg-transparent text-themeAccent text-xl lg:text-2xl`}>
 <i className="fa-solid fa-wallet"></i>
 </div>
 {currentTotal > 0 ? (
 <span className="px-3 py-1.5 bg-transparent text-rose-400 border border-black/5 dark:border-white/10 rounded-lg text-[9px] lg:text-[13px] font-medium">
 Dues Pending
 </span>
 ) : (
 <span className="px-3 py-1.5 bg-transparent text-emerald-400 border border-black/5 dark:border-white/10 rounded-lg text-[9px] lg:text-[13px] font-medium">
 All Clear
 </span>
 )}
 </div>
 ...
 <h2 className={`text-4xl lg:text-5xl font-black tracking-tight mb-2 transition-colors duration-300 ${currentTotal === 0 ? 'text-themeTextSec dark:text-white/50' : 'text-themeText dark:text-white'}`}>
 ...
 : 'bg-transparent text-themeTextSec dark:text-white/50 cursor-not-allowed border border-transparent'
 }`}
 >
 ...
 <h2 className={`${theme.text.heading} text-lg lg:text-xl text-themeText dark:text-white tracking-tight ml-2`}>Detailed Breakdown</h2>
 ...
 <p className="text-themeTextSec dark:text-white/50 font-bold text-xs lg:text-sm">No fee records found in your ledger.</p>
 </div>
 ) : (
 <div className="bg-themePanel border border-black/5 dark:border-white/10 rounded-[2.5rem] overflow-hidden p-4 lg:p-6">
 ...
 className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 lg:p-5 rounded-2xl lg:rounded-2xl transition duration-300 mb-2 last:mb-0 border-themeBorder dark:border-white/5 ${item.status === 'paid'
 ? 'opacity-50 grayscale bg-white/5 backdrop-blur-sm border-themeBorder dark:border-white/5'
 : `bg-black/5 dark:bg-white/5 backdrop-blur-xl border border-black/10 dark:border-white/5 hover:bg-white/20 hover:border-white/30 cursor-pointer hover:border-amber-500/40 hover:-translate-y-0.5 ${isSelected ? 'border-amber-500/30 bg-amber-500/10' : 'border-transparent'}`
 }`}
 >
 <div className="flex items-center gap-3 lg:gap-4">
 <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center shrink-0 border-themeBorder dark:border-white/5 ${getFeeTheme(item.type)}`}>
 <i className={`fa-solid ${getFeeIcon(item.type)} text-base lg:text-lg`}></i>
 </div>
 <div>
 <h3 className={`text-sm lg:text-base font-black tracking-tight ${item.status === 'paid' ? 'line-through text-themeTextSec dark:text-white/50' : 'text-themeText dark:text-white'}`}>
 {item.title}
 </h3>
 ...
 <span className={`text-base lg:text-xl font-semibold tracking-tight ${item.status === 'paid' ? 'text-themeTextSec dark:text-white/50' : 'text-themeText dark:text-white'}`}>
 {formatCurrency(item.amount)}
 </span>

 {item.status === 'paid' ? (
 <span className="text-emerald-400 text-[9px] lg:text-[13px] font-medium flex items-center gap-1.5 bg-themePanel border border-black/5 dark:border-white/10 border border-black/5 dark:border-white/10 px-2.5 lg:px-3 py-1.5 rounded-lg">
 ...
 <p className="text-themeTextSec dark:text-white/50 font-bold text-xs lg:text-sm">No transactions have been recorded yet.</p>
 </div>
 ) : (
 <div className="bg-themePanel border border-black/5 dark:border-white/10 rounded-[2.5rem] overflow-hidden p-4 lg:p-6">
 ...
 <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center shrink-0 border-themeBorder dark:border-white/5 ${txn.status === 'successful' ? 'bg-themePanel border border-black/5 dark:border-white/10 text-emerald-400 border-themeBorder dark:border-white/5' : 'bg-themePanel border border-black/5 dark:border-white/10 text-rose-400 border-themeBorder dark:border-white/5'
 }`}>
 <i className={`fa-solid ${txn.status === 'successful' ? 'fa-arrow-down' : 'fa-xmark'} text-base lg:text-lg`}></i>
 </div>
 <div className="flex-1">
 <h3 className="text-sm lg:text-base font-black text-themeText dark:text-white tracking-tight leading-tight mb-1 truncate max-w-[200px] sm:max-w-md lg:max-w-xl" title={txn.purpose}>{txn.purpose}</h3>
 ...
 <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-2 lg:gap-3 border-themeBorder dark:border-white/5 md:border-0 border-transparent pt-3 md:pt-0">
 <span className="text-base lg:text-xl font-semibold tracking-tight text-themeText dark:text-white">{formatCurrency(txn.amount)}</span>
 <div className="flex items-center gap-2 lg:gap-3">
 <span className={`text-[8px] lg:text-[12px] font-medium px-2.5 py-1.5 rounded-lg border-themeBorder dark:border-white/5 ${txn.status === 'successful' ? 'bg-themePanel border border-black/5 dark:border-white/10 text-emerald-400 border-themeBorder dark:border-white/5' : 'bg-themePanel border border-black/5 dark:border-white/10 text-rose-400 border-themeBorder dark:border-white/5'
 }`}>
 {txn.status}
 </span>
 {txn.status === 'successful' && (
 <button type="button" onClick={() => handleDownloadInvoice(txn)} disabled={downloadingInvoiceId === txn.id} className="text-themeTextSec dark:text-white/50 hover:text-themeAccent transition duration-300 bg-transparent hover:bg-themePanel border border-black/5 dark:border-white/10 w-8 h-8 lg:w-9 lg:h-9 rounded-lg lg:rounded-2xl flex items-center justify-center border-themeBorder dark:border-white/5 disabled:opacity-50 disabled:cursor-not-allowed no-print" title="Download Receipt">
