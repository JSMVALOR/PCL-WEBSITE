...
export default function MootCourt({ isEmbedded = false, }) {
 ...
 try {
 const studentId = userSession?.db_id || userSession?.id;
 const { error } = await supabase.from('moot_bids').insert({ student_id: studentId,
 moot_id: mootId,
 ...
 const { error } = await supabase.from('memorial_vault').insert({ author_id: studentId,
 ...
 <p className="text-2xl lg:text-3xl font-semibold tracking-tight text-themeText tracking-tight">{s.value}</p>
 <p className={`text-[9px] lg:text-[10px] font-bold ${theme.text.muted} tracking-normal`}>{s.label}</p>
 </div>
 ))}
 </div>
 ...
 <span className={`px-2 py-1 rounded-md text-[8px] lg:text-[12px] font-medium border-theme ${getLevelTheme(moot.level)}`}>
 {moot.level}
 </span>
 <span className={`text-[8px] lg:text-[12px] font-medium px-2 py-1 rounded-md border-theme ${getStatusTheme(moot.status)}`}>
 {moot.status}
 </span>
 </div>
 <h4 className="text-base lg:text-lg font-semibold tracking-tight text-themeText group-hover:text-amber-400 transition-colors leading-tight">{moot.moot_name}</h4>
 </div>
 ...
 <p className="text-[8px] lg:text-[12px] font-medium text-themeTextSec opacity-70 mb-0.5"><i className="fa-solid fa-calendar mr-1"></i> Date</p>
 ...
 <div className="bg-transparent rounded-lg p-2.5 border border-black/10 dark:border-white/20">
 <p className="text-[8px] lg:text-[12px] font-medium text-themeTextSec opacity-70 mb-0.5"><i className="fa-solid fa-location-dot mr-1"></i> Venue</p>
 ...
 <h5 className="text-[10px] lg:text-[14px] font-medium tracking-normal text-themeText mb-3 flex items-center gap-2">
 <i className="fa-solid fa-feather text-amber-500"></i> Submit Blind Bid
 </h5>
 
 {submitSuccess === moot.id ? (
 <div className="w-full py-6 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[14px] font-medium tracking-normal flex items-center justify-center gap-2">
 ...
 <button type="submit" disabled={isSubmitting} className="px-6 py-3 bg-amber-500 hover:bg-amber-400 text-[#050505] rounded-[2rem] text-[13px] font-medium transition active:scale-[0.98] disabled:opacity-50 flex items-center gap-2">
 ...
 <div>
 <span className="px-3 py-1 bg-amber-500/10 text-amber-500 border-theme border-amber-500/20 rounded-md text-[12px] font-medium mb-3 inline-block">Official ISC Standing</span>
 <h2 className="text-3xl lg:text-5xl font-black tracking-tight text-themeText mb-1">Rank #{stats.rank.replace('#', '')}</h2>
 <p className={`${theme.text.muted} text-xs lg:text-sm font-bold tracking-normal mt-2`}><i className="fa-solid fa-chart-line mr-1"></i> {iscBreakdown.total_points} cumulative points</p>
 </div>
 ...
 <p className="text-[8px] lg:text-[12px] font-medium text-themeTextSec opacity-70 mb-1.5">{s.label}</p>
 <p className={`text-xl lg:text-3xl font-semibold tracking-tight ${s.color}`}>{s.value}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 <h3 className="text-base lg:text-lg font-semibold tracking-tight text-themeText mb-5 flex items-center gap-2">
 ...
 <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-[15px] font-semibold border-2 ${round.passed ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-rose-500/10 border-rose-500 text-rose-400'}`}>
 ...
 <p className={`text-[9px] font-bold ${theme.text.muted} tracking-normal`}>{round.date}</p>
 </div>
 <span className="text-[14px] font-medium text-themeText bg-themePanel border-theme border-themeBorderStrong px-3 py-1.5 rounded-lg shrink-0 flex items-center gap-1">
 ...
 <div>
 <h2 className="text-xl lg:text-2xl font-semibold tracking-tight text-purple-100 tracking-tight mb-1">Memorial Archive</h2>
 ...
 <button type="button" onClick={() => setShowMemorialModal(true)} className="w-full md:w-auto px-6 py-3.5 bg-purple-600 hover:bg-purple-500 text-gray-900 dark:text-white rounded-[2rem] text-[10px] lg:text-[14px] font-medium tracking-normal transition active:scale-[0.98] flex items-center justify-center gap-2 shrink-0 border-theme border-purple-400/50">
 ...
 <span className="text-[8px] lg:text-[9px] font-black text-purple-300 bg-purple-500/10 px-2 py-1 rounded-md border-theme border-purple-500/20 shrink-0 tracking-normal">{memo.year_submitted}</span>
 {memo.is_verified && <span className="text-[8px] lg:text-[9px] font-black text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-md border-theme border-emerald-500/20 tracking-normal flex items-center gap-1"><i className="fa-solid fa-check"></i> Verified</span>}
 </div>
 ...
 <p className={`text-[9px] lg:text-[10px] font-bold ${theme.text.secondary} tracking-normal truncate`}><i className="fa-solid fa-gavel text-neutral-500 mr-1"></i> {memo.competition_name}</p>
 </div>
 </div>
 ...
 <p className={`font-bold text-[9px] lg:text-[10px] tracking-normal mb-2 lg:mb-3 flex items-center gap-2 ${i === 0 ? 'text-amber-500' : 'text-themeTextSec opacity-70'}`}>
 ...
 <div>
 <h3 className="text-lg lg:text-xl font-semibold tracking-tight text-purple-50 tracking-tight mb-1 flex items-center gap-2">
 <i className="fa-solid fa-cloud-arrow-up text-purple-400"></i> Deposit to Archive
 </h3>
 <p className="text-[10px] lg:text-xs text-purple-300/80 font-bold tracking-normal"><i className="fa-solid fa-shield-halved mr-1"></i> Undergoes plagiarism & quality review</p>
 </div>
 <button type="button" onClick={() => setShowMemorialModal(false)} className="w-8 h-8 rounded-full bg-purple-500/10 border-theme border-purple-500/30 text-purple-300 hover:text-gray-900 dark:text-white hover:bg-purple-500/30 flex items-center justify-center transition-colors"><i className="fa-solid fa-xmark"></i></button>
 </div>
 </div>
 ...
 <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[13px] font-medium flex items-center justify-center gap-2">
 <i className="fa-solid fa-check-circle text-lg"></i> Deposited Successfully
 </div>
 ) : (
 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-purple-600 hover:bg-purple-500 text-gray-900 dark:text-white rounded-[2rem] text-[10px] lg:text-[14px] font-medium tracking-normal transition active:scale-[0.98] disabled:opacity-50 mt-2">
