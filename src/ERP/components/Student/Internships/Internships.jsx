...
// --- SHARED STYLES ---
const INPUT_CLS = "w-full bg-themePanel border-theme border-themeBorderStrong rounded-[2rem] px-4 py-3 text-xs lg:text-sm font-bold text-themeText outline-none focus:border-themeAccent transition";
...
export default function Internships({ isEmbedded = false }) {
 ...
 default: return "bg-themePanel border-theme border-themeBorderStrong text-themeTextSec border-black/10 dark:border-white/20";
 }
 };
 ...
 <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b-theme border-black/10 dark:border-white/20 pb-4">
 <div>
 ...
 <div className="col-span-full py-16 lg:py-24 text-center border-2 border-dashed border-black/10 dark:border-white/20 rounded-[2rem] bg-white/5 backdrop-blur-[80px] backdrop-blur-sm px-4">
 ...
 return (
 <div key={log.id} className={`${theme.layout.panel} rounded-[2rem] border-theme ${isExpanded ? 'border-black/5 dark:border-white/10' : 'border-black/10 dark:border-white/20'} hover:border-black/5 dark:border-white/10 transition group flex flex-col`}>
 ...
 <div className={`w-6 h-6 rounded-full flex items-center justify-center bg-themePanel border-theme border-themeBorderStrong text-themeTextSec transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
 ...
 <span className="flex items-center gap-1.5 bg-themePanel border-theme border-themeBorderStrong px-2.5 py-1 rounded-md border border-black/10 dark:border-white/20"><i className="fa-regular fa-calendar text-themeAccent"></i> {log.duration}</span>
 <span className="flex items-center gap-1.5 bg-themePanel border-theme border-themeBorderStrong px-2.5 py-1 rounded-md border border-black/10 dark:border-white/20"><i className="fa-solid fa-location-dot text-themeAccent"></i> {log.location}</span>
 </div>
 </div>

 {isExpanded && (
 <div className="border-t-theme border-black/10 dark:border-white/20 animate-fade-in bg-themeElevated/20 rounded-b-themePanel">
 ...
 <p className="text-[11px] lg:text-xs text-themeTextSec leading-relaxed bg-themePanel border-theme border-themeBorderStrong p-4 rounded-[2rem] italic">
 "{log.description}"
 </p>
 </div>
 ...
 <p className="text-[11px] lg:text-xs text-themeTextSec leading-relaxed bg-emerald-500/5 p-4 rounded-[2rem] border-theme border-emerald-500/20">
 {log.certificate_notes}
 </p>
 </div>
 )}
 ...
 {dailyLogs.length === 0 ? (
 <div className="py-6 text-center bg-themePanel border-theme border-themeBorderStrong rounded-[2rem]">
 ...
 <div key={i} className="flex items-start gap-3 bg-themePanel border-theme border-themeBorderStrong p-3.5 rounded-[2rem] hover:border-black/5 dark:border-white/10 transition-colors">
 ...
 <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded border-theme ${hoursProgress >= 100 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-themePanel border-theme border-themeBorderStrong text-themeTextSec border-black/10 dark:border-white/20'}`}>
 {hoursProgress >= 100 ? 'Completed' : 'In Progress'}
 </span>
 </div>
 <div className="h-2.5 lg:h-3 w-full bg-themePanel border-theme border-themeBorderStrong rounded-full overflow-hidden border border-black/10 dark:border-white/20">
 ...
 <div key={log.id} className={`${theme.layout.panel} p-5 lg:p-6 rounded-[2rem] flex flex-col justify-between hover:border-black/5 dark:border-white/10 transition group`}>
 <div>
 <div className="flex justify-between items-start mb-4">
 <span className={`px-2.5 py-1 rounded-md text-[8px] lg:text-[9px] font-black uppercase tracking-widest border-theme ${getTypeTheme(log.type)}`}>
 {log.type}
 </span>
 <span className="text-[10px] lg:text-xs font-black text-themeText bg-themePanel border-theme border-themeBorderStrong px-3 py-1 rounded-md border border-black/10 dark:border-white/20">{log.hours} Hours</span>
 </div>
 ...
 <p className="text-[10px] lg:text-xs font-medium text-themeTextSec bg-themeElevated/50 p-3 lg:p-4 rounded-[2rem] border-l-2 border-themeAccent italic leading-relaxed line-clamp-3">
 "{log.description}"
 </p>
 </div>
 <div className="mt-5 pt-4 border-t-theme border-black/10 dark:border-white/20 flex items-center justify-between">
 ...
 <div className="flex justify-between items-center border-b-theme border-black/10 dark:border-white/20 pb-4 px-2">
 <div>
 ...
 <div className="w-full py-16 lg:py-24 text-center border-2 border-dashed border-black/10 dark:border-white/20 rounded-[2rem] bg-white/5 backdrop-blur-[80px] backdrop-blur-sm px-4">
 ...
 nocRequests.map((req) => (
 <div key={req.id} className={`${theme.layout.panel} p-5 lg:p-6 rounded-[2rem] flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:border-black/5 dark:border-white/10 transition group`}>
 ...
 <p className={`text-[10px] lg:text-xs font-bold text-themeTextSec mb-3 bg-themePanel border-theme border-themeBorderStrong px-3 py-1.5 rounded-lg w-fit`}><i className="fa-regular fa-calendar mr-1.5 text-themeAccent"></i> {req.duration}</p>
 ...
 {/* Approval Pipeline */}
 <div className="w-full lg:w-auto bg-themeElevated/50 border border-black/10 dark:border-white/20 rounded-[2rem] p-4 flex items-center justify-between sm:justify-center gap-3 sm:gap-6 shrink-0">
 ...
 ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400'
 : req.status === 'pending_hod' ? 'bg-amber-500/10 border-amber-500 text-amber-400' : 'bg-themePanel border-theme border-themeBorderStrong border-black/5 dark:border-white/10 text-neutral-600'
 }`}>
 ...
 <div className="flex items-center gap-3 bg-[#0a66c2]/10 p-4 rounded-[2rem] border-theme border-[#0a66c2]/20 cursor-pointer hover:bg-[#0a66c2]/20 transition-colors" onClick={() => setShareExpOnSubmit(!shareExpOnSubmit)}>
 <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${shareExpOnSubmit ? 'bg-[#0a66c2] text-white' : 'bg-themePanel border-theme border-themeBorderStrong'}`}>
 ...
 {submitSuccess ? (
 <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"><i className="fa-solid fa-check-circle text-lg"></i> Experience Logged</div>
 ) : (
 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-themeText hover:bg-themeText/90 text-themePanel rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition active:scale-[0.98] disabled:opacity-50">{isSubmitting ? "Writing to Ledger..." : "Log Experience"}</button>
 )}
 </form>
 </div>
 </div>
 ...
 <i className="fa-brands fa-google-drive text-themeTextSec"></i>
 </div>
 <input 
 type="url" 
 ...
 {submitSuccess ? (
 <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2"><i className="fa-solid fa-check-circle text-lg"></i> NOC Routed to Mentor</div>
 ) : (
 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-themeText hover:bg-themeText/90 text-themePanel rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition active:scale-[0.98] disabled:opacity-50">{isSubmitting ? "Routing to Mentor..." : "Submit NOC Request"}</button>
 )}
 </form>
 </div>
 </div>
 ...
 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 text-[#050505] rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition active:scale-[0.98] disabled:opacity-50">{isSubmitting ? "Logging..." : "Submit Practical Log"}</button>
 )}
 </form>
 </div>
 </div>
 ...
 <div className="bg-themePanel border-theme border-themeBorderStrong p-5 lg:p-6 border-b-theme border-black/10 dark:border-white/20 relative overflow-hidden">
 <div className="absolute top-0 right-0 w-24 h-24 bg-themePanel border-theme border-themeBorderStrong rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
 <div className="relative z-10 flex justify-between items-start">
 <div>
 ...
 <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition active:scale-[0.98] disabled:opacity-50">{isSubmitting ? "Saving..." : "Add Log Entry"}</button>
 )}
 </form>
 </div>
 </div>
