...
// Find the map block
const startText = '{filteredStudents.map((student, index) => {';
 ...
 >
    <div className="flex items-center gap-4 w-full cursor-pointer pl-2">
        ...
        <div className="shrink-0 flex gap-2">
            {currentStatus === 'present' && <span className="bg-emerald-500/20 text-emerald-500 px-3 py-1 rounded-full text-xs font-bold">Present</span>}
            {currentStatus === 'absent' && <span className="bg-rose-500/20 text-rose-500 px-3 py-1 rounded-full text-xs font-bold">Absent</span>}
            {(currentStatus === 'medical' || currentStatus === 'early_leave') && <span className="bg-amber-500/20 text-amber-500 px-3 py-1 rounded-full text-xs font-bold">{currentStatus === 'medical' ? 'Medical' : 'Left Early'}</span>}
            {currentStatus === 'late' && <span className="bg-emerald-500/20 text-emerald-500 px-3 py-1 rounded-full text-xs font-bold">Late</span>}
        </div>
    </div>
 </SwipeRow>
 );
